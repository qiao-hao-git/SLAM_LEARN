...
void bundleAdjustmentG2o(const VecVector2d &points_2d, const VecVector3d &points_3d, const Mat &K, Sophus::SE3d &pose)
{
...
void bundleAdjustmentGaussNewton(const VecVector2d &points_2d, const VecVector3d &points_3d, const Mat &K, Sophus::SE3d &pose)
{
    typedef Eigen::Matrix<double, 6, 1> Vector3d;
...
int main(int argc, char **argv)
{
    ...
    {
        ushort d = d1.ptr<unsigned short>(int(key_points_1[m.queryIdx].pt.y))[int(key_points_1[m.queryIdx].pt.x)];
        if(d == 0)
        {
            continue;
        }
        ...
        pts_2d.push_back(key_points_2[m.queryIdx].pt);
    }
    ...
    cout << "solve pnp by gauss newton const time:" << time_used.count() << "seconds" << endl;
    ...
    t2 = chrono::steady_clock::now();
    time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
    cout << "solve pnp by g2o cost time:" << time_used.count() << endl;
