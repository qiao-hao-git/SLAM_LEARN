...
string first_map = "./1.jpg";
string second_map = "./2.jpg";
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches)
{
    ...
    for(size_t i1 = 0; i1 < desc1.size(); i1++)
    {
        ...
        for(size_t i2 = 0; i2 < desc2.size(); i2++)
        {
            ...
            for(int k = 0; k < 8; k++)
            {
                distance += nums_of_1(desc1[i1][k] ^ desc2[i2][k]);
            }
...
{
    cv::Mat first_image = cv::imread(first_map, 0);
    cv::Mat second_image = cv::imread(second_map, 0);
